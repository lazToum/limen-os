...
def render_md(text: str) -> str:
    ...
    md = markdown.Markdown(
        extensions=[
            ...
            "attr_list",
            CodeHiliteExtension(
                css_class="highlight", guess_lang=True, noclasses=False
            ),
        ]
    )
    ...
    for item in NAV:
        ...
        if len(item) == 2:
            ...
            for child_label, child_md in children:
                ...
                cls = (
                    "active"
                    if child_md == active_md and subdir == active_subdir
                    else ""
                )
...
def build(site_dir: Path = SITE) -> None:
    ...
    if landing.exists():
        ...
        print("  landing  → static/docs/index.html")
    ...
    for subdir, md_name, md_path in collect_pages():
        ...
        title = extract_title(
            text, md_name.replace(".md", "").replace("-", " ").title()
        )
        body = render_md(text)
...
def build_rust(site_dir: Path = SITE) -> None:
    ...
    try:
        r = subprocess.run(
            ["cargo", "doc", "--no-deps", "--workspace"], cwd=rust_dir, check=False
        )
    ...
    if doc_src.is_dir():
        ...
        print("  rustdoc  → static/docs/api/rust/")
...
def _start_watcher() -> None:
    ...
    if HAS_WATCHDOG:
        ...
        print(
            "  [reload] watchdog not installed, falling back to polling (pip install watchdog)"
        )
...
def _poll_and_rebuild(interval: float = 1.0) -> None:
    def _snapshot() -> dict:
        ...
        for d in WATCH_DIRS:
            if d.exists():
                result.update(
                    {p: p.stat().st_mtime for p in d.rglob("*") if p.is_file()}
                )
        return result
...
def serve(port: int = 8001, full: bool = False, reload: bool = False) -> None:
    ...
    class _Handler(handler):
        def log_message(self, fmt, *args):
            ...
            if not any(
                x in msg for x in (".js", ".css", ".woff", ".png", ".ico", ".svg")
            ):
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--full", action="store_true", help="also build rustdoc and typedoc"
    )
    parser.add_argument(
        "--reload",
        action="store_true",
