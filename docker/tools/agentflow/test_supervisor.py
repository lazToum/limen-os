...
from agentflow.core.actor import Actor, Message, ActorState, SupervisorStrategy
...
def make_system(poll_interval: float = 0.05):
    ...
    system._supervisor = Supervisor(
        system.registry, system._inject, poll_interval=poll_interval
    )
    return system
...
async def test_one_for_one_restart():
    ...
    assert_true(
        "crash-once was restarted (factory called >1 time)", call_n["crash"] >= 2
    )
...
async def test_budget_exhausted_gives_up():
    ...
    system.registry.find_by_name = lambda n: (
        FakeMain() if n == "main" else original_find(n)
    )
...
async def test_stop_all_stops_supervisor():
    ...
    assert_true(
        "watch task is running before stop_all", watch_task and not watch_task.done()
    )
    ...
    assert_true(
        "watch task is cancelled after stop_all", watch_task and watch_task.done()
    )
