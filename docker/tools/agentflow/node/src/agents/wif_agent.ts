...
export class WifAgent extends Actor {
  ...
  private _dispatch(cmd: string, args: string[]): string {
    switch (cmd) {
      ...
      case "budget":
        return this._cmdBudget(args);
      case "report":
        return this._cmdReport();
      case "balance":
        return this._cmdBalance();
      case "compound":
        return this._cmdCompound(args);
      case "loan":
        return this._cmdLoan(args);
      case "roi":
        return this._cmdRoi(args);
      case "tax":
        return this._cmdTax(args);
      case "tip":
        return this._cmdTip(args);
      case "help":
      case "":
        return HELP;
      default:
  ...
  private _cmdAdd(args: string[]): string {
    ...
    const header =
      `Logged **$${amount.toFixed(2)}** → \`${category}\`` +
      (note ? ` — _${note}_` : "");
    if (this._budget > 0) {
  ...
  private _cmdTip(args: string[]): string {
    ...
    const splits = [1, 2, 3, 4, 5]
      .map((n) => `  ${n} people: $${(total / n).toFixed(2)}/person`)
      .join("\n");
