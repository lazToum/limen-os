...
export class HomeAssistantAgent extends Actor {
  ...
  private get _configured(): boolean {
  ...
  private async _dispatch(text: string): Promise<void> {
    ...
    switch (cmd) {
      ...
      case "status":
        ...
        break;
      case "devices":
        await this._cmdDevices();
        break;
      case "entities":
        await this._cmdEntities();
        break;
      case "domains":
        await this._cmdDomains();
        break;
      case "search":
        await this._cmdSearch(tokens.slice(1).join(" "));
        break;
      case "state":
        await this._cmdState(tokens[1] ?? "");
        break;
      default:
  ...
  private async _cmdDevices(): Promise<void> {
    if (!this._configured) {
      this.replyChat("HA not configured.");
      return;
    }
    try {
      await this._ensureCache();
      if (!this._devices.length) {
        this.replyChat("No devices found.");
        return;
      }
      ...
      for (const d of this._devices.slice(0, 25)) {
        ...
        if (d.manufacturer || d.model)
          line += ` (${[d.manufacturer, d.model].filter(Boolean).join(" ")})`;
        if (d.area_id) line += ` — area: ${d.area_id}`;
        lines.push(line);
      }
      if (this._devices.length > 25)
        lines.push(`… and ${this._devices.length - 25} more`);
  ...
  private async _cmdEntities(): Promise<void> {
    ...
    try {
      ...
      if (!this._entities.length) {
        this.replyChat("No entities found.");
        return;
      }
      ...
      for (const e of this._entities.slice(0, 30)) {
        ...
        lines.push(
          name ? `- \`${e.entity_id}\` — ${name}` : `- \`${e.entity_id}\``,
        );
      }
      if (this._entities.length > 30)
        lines.push(
          `… and ${this._entities.length - 30} more. Use \`search <keyword>\` to filter.`,
        );
      this.replyChat(lines.join("\n"));
    } catch (e) {
      this.replyChat(`✗ ${e}`);
  ...
  private async _cmdDomains(): Promise<void> {
    ...
    try {
      ...
      for (const [domain, count] of sorted)
        lines.push(`- \`${domain}\` — ${count} entities`);
      this.replyChat(lines.join("\n"));
    } catch (e) {
      this.replyChat(`✗ ${e}`);
    }
  ...
  private async _cmdSearch(keyword: string): Promise<void> {
    if (!keyword) {
      this.replyChat("Usage: `search <keyword>`");
      return;
    }
    if (!this._configured) {
      this.replyChat("HA not configured.");
    ...
    try {
      ...
      const matches = this._entities.filter(
        (e) =>
          e.entity_id.toLowerCase().includes(kw) ||
          (e.original_name ?? e.name ?? "").toLowerCase().includes(kw),
      );
      if (!matches.length) {
        this.replyChat(`No entities matching \`${keyword}\`.`);
        return;
      }
      const lines = [
        `**Search: \`${keyword}\`** — ${matches.length} match(es):\n`,
      ];
  ...
  private async _cmdState(entityId: string): Promise<void> {
    if (!entityId) {
      this.replyChat(
        "Usage: `state <entity_id>`  e.g. `state light.living_room`",
      );
      return;
    }
    try {
      const s = (await this._haGet(`/api/states/${entityId}`)) as Record<
        string,
        unknown
      >;
      ...
      this.replyChat(`✗ Could not get state for \`${entityId}\`: ${e}`);
    }
  }
}
