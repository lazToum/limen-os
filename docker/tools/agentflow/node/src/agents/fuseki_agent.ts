...
export class FusekiAgent extends Actor {
  ...
  private async _dispatch(text: string): Promise<void> {
    ...
    if (lower === "datasets") {
      await this._cmdDatasets();
      return;
    }
    if (lower.startsWith("ask ")) {
      await this._cmdAsk(text.slice(4).trim());
      return;
    }
    if (lower.startsWith("query ")) {
      await this._cmdQuery(text.slice(6).trim());
      return;
    }
    // Raw SPARQL
    const up = text.trimStart().toUpperCase();
    if (
      ["SELECT", "CONSTRUCT", "DESCRIBE", "PREFIX", "ASK"].some((k) =>
        up.startsWith(k),
      )
    ) {
      await this._cmdQuery(text);
      return;
    }
  ...
  private async _cmdQuery(sparql: string): Promise<void> {
    if (!sparql) {
      this.replyChat("Usage: `query <sparql>`");
      return;
    }
    try {
      ...
      const msg = axios.isAxiosError(e)
        ? `HTTP ${e.response?.status}: ${String(e.response?.data ?? e.message).slice(0, 300)}`
        : String(e);
  ...
  private async _cmdAsk(sparql: string): Promise<void> {
    const q = sparql.trimStart().toUpperCase().startsWith("ASK")
      ? sparql
      : `ASK { ${sparql} }`;
    try {
      const resp = await axios.get(SPARQL_ENDPOINT, {
      ...
      this.replyChat(
        `**ASK Result:** ${bool ? "✓" : "✗"} \`${bool ? "true" : "false"}\`\n\nQuery: \`${q}\``,
      );
    } catch (e: unknown) {
  ...
  private async _cmdDatasets(): Promise<void> {
    try {
      ...
      const datasets =
        ((resp.data as Record<string, unknown>)["datasets"] as unknown[]) ?? [];
      if (!datasets.length) {
        this.replyChat("No datasets found.");
        return;
      }
  ...
  private _formatResults(data: Record<string, unknown>): string {
    ...
    if (!bindings.length)
      return `Query returned 0 rows. Columns: ${vars.join(", ")}`;
    const lines = [
      `**Results** (${bindings.length} rows, columns: ${vars.join(", ")}):\n`,
    ];
    ...
    if (bindings.length > 20)
      lines.push(`… and ${bindings.length - 20} more rows`);
    return lines.join("\n");
  }
}
