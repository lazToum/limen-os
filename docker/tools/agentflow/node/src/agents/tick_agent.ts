...
export class TickAgent extends Actor {
  ...
  private async _dispatch(
    ...
    switch (cmd) {
      ...
      case "rm":
      case "del":
        return this._cmdCancel(args[0] ?? "");
      case "at":
        return args.length < 2
          ? "Usage: `at <HH:MM> <message>`"
          : this._cmdAt(args[0], args.slice(1).join(" "));
      case "in":
        return args.length < 3
          ? "Usage: `in <n> <unit> <message>`"
          : this._cmdIn(args[0], args[1], args.slice(2).join(" "));
      case "every":
        return args.length < 3
          ? "Usage: `every <n> <unit> <message>`"
  ...
  private _cmdAt(timeStr: string, message: string): string {
    ...
    if (!fireAt)
      return `Invalid time \`${timeStr}\`. Use HH:MM (24-h), e.g. \`14:30\`.`;
    const id = shortId();
    const delayMs = fireAt - Date.now();
    const h = Math.floor(delayMs / 3600000),
      m = Math.floor((delayMs % 3600000) / 60000);
  ...
  private _cmdIn(n: string, unit: string, message: string): string {
    ...
    if (!ms)
      return `Invalid delay \`${n} ${unit}\`. Use e.g. \`5 m\`, \`2 h\`, \`30 s\`.`;
    const id = shortId();
  ...
  private _cmdEvery(n: string, unit: string, message: string): string {
    const ms = parseMs(n, unit);
    if (!ms || ms < 60000)
      return `Invalid interval \`${n} ${unit}\`. Minimum is 1 minute.`;
    const id = shortId();
  ...
  private _cmdList(): string {
    ...
    const sorted = [...this._timers.values()].sort(
    ...
    for (const t of sorted) {
      ...
      const label =
        remaining < 60000
          ...
          : remaining < 3600000
            ? `in ${Math.ceil(remaining / 60000)}m`
            : `in ${(remaining / 3600000).toFixed(1)}h`;
      lines.push(
        `- \`${t.id}\` [${kind}] ${label} — _${t.message.slice(0, 60)}_`,
      );
    }
    return lines.join("\n");
  }
  ...
  private _cmdCancel(prefix: string): string {
    ...
    const matches = [...this._timers.keys()].filter((id) =>
      id.startsWith(prefix),
    );
    if (!matches.length) return `No timer found matching \`${prefix}\`.`;
    if (matches.length > 1)
      return `Ambiguous prefix \`${prefix}\` — ${matches.length} matches. Be more specific.`;
