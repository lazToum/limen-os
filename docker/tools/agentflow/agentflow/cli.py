...
def get_args():
    ...
    parser.add_argument(
        "--llm", choices=["anthropic", "openai", "ollama", "nim", "none"]
    )
...
async def _start_web_ui(
    port: int, mqtt_broker: str, mqtt_port: int, actor_registry=None
) -> None:
...
async def build_system(args: argparse.Namespace):
    ...
    from agentflow.agents.llm_agent import (
        AnthropicProvider,
        OpenAIProvider,
        OllamaProvider,
        NIMProvider,
    ...
    from agentflow.agents.home_assistant_state_bridge_agent import (
    ...
    system._mqtt_client = await __import__(
        ...
        args.mqtt_broker or CONFIG.mqtt_host, args.mqtt_port or CONFIG.mqtt_port
    )
    ...
    def make_main():
        return MainActor(
            llm_provider=make_provider(), name="main", persistence_dir="./state"
        )
    ...
    def make_manual_agent():
        return ManualAgent(
            llm_provider=make_provider(), name="manual-agent", persistence_dir="./state"
        )
    ...
    def make_anomaly_agent():
        return AnomalyDetectorAgent(
            name="anomaly-detector", continuous=False, persistence_dir="./state"
        )
