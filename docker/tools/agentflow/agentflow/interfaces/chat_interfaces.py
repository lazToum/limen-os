...
REMOTE_RUNNER_PATH = os.path.normpath(
    os.path.join(_HERE, "..", "..", "remote_runner.py")
)
...
""")
    ...
    async def _get_agent_response(self, agent_name: str, message: str) -> str:
        ...
        if target is None:
            ...
            return (
                f"[error] No agent named '{agent_name}'. Available: {', '.join(names)}"
            )
    ...
    async def _get_remote_agent_response(self, agent_name: str, message: str) -> str:
        ...
        if not remote_node:
            known = [
                a for nd in main._known_nodes.values() for a in nd.get("agents", [])
            ]
            if known:
    ...
    async def _discover_host(self, node_name: str) -> str:
        ...
        for hostname in candidates:
            try:
                ...
                ans = await asyncio.get_event_loop().run_in_executor(
                    None,
                    lambda h=hostname, i=ip: (
                        input(f"  Use {h} ({i})? [Y/n]: ").strip().lower()
                    ),
                )
                if ans in ("", "y", "yes"):
                    return hostname
        ...
        if found:
            ...
            ans = await asyncio.get_event_loop().run_in_executor(
                None,
                lambda: input(f"  Pick [1-{len(found)}] or type IP manually: ").strip(),
            )
    ...
    async def _deploy(self, node_name: str, host: str = ""):
        ...
        password = await loop.run_in_executor(
            None,
            lambda: __import__("getpass").getpass(
        ...
        broker = await loop.run_in_executor(
            None,
            lambda: (
                input("  MQTT broker IP (this machine's LAN IP): ").strip()
                or "localhost"
            ),
        )
        ...
        if not hasattr(self.agent, "delegate_to_installer"):
            print(
                "[error] delegate_to_installer not available. Is the installer agent running?"
            )
            return
            ...
            try:
                ...
                if text.lower() in ("/nodes", "nodes"):
                    ...
                    for nd in sorted(remote_nodes, key=lambda x: x["node"]):
                        ...
                        names = (
                            ", ".join("@" + n for n in nd["agents"]) or "(no agents)"
                        )
                ...
                if text.lower().startswith("/deploy-pkg"):
                    ...
                    if len(parts) < 3:
                        ...
                        password = await loop.run_in_executor(
                            None,
                            lambda: __import__("getpass").getpass("  SSH password: "),
                        )
                ...
                if text.startswith("@"):
                    ...
                    if target and hasattr(target, "chat_stream"):
                        ...
                        response = await self._get_remote_agent_response(
                            agent_name, message
                        )
                        print(f"\n@{agent_name}: {response}\n")
                    continue
...
class RESTInterface:
    ...
    async def run(self):
        ...
        async def command_endpoint(request):
            ...
            from ..core.actor import MessageType  # noqa: PLC0415
