...
class _RemoteAgentAPI:
    ...
    async def send_to(
        self, agent_name: str, payload: Any, timeout: float = 30.0
    ) -> Any:
        """
...
class _RemoteAgent:
    ...
    def __init__(self, config: dict, runner: "_RemoteRunner"):
        ...
        self.actor_id = str(
            uuid.uuid5(uuid.NAMESPACE_DNS, f"agentflow.actor.{self.name}")
        )
    ...
    async def _supervisor_loop(self):
        ...
        while self._running and not self._failed:
            try:
                ...
                delay = min(
                    self._restart_delay * (2 ** (self._restart_count - 1)), 60.0
                )
                logger.warning(
    ...
    async def _process_loop(self):
        ...
        while self._running:
            try:
                ...
                logger.error(
                    f"[{self.name}] process() error #{consecutive_errors}: {e}"
                )
                await self._publish(
                    f"agents/{self.actor_id}/errors",
                    {
...
class _RemoteRunner:
    ...
    async def spawn_agent(self, config: dict):
        if not isinstance(config, dict):
            logger.warning(
                f"[runner] spawn_agent: invalid config type {type(config)}, ignoring."
            )
            return
        ...
        if name in self._agents:
            if config.get("replace", False):
                ...
                logger.info(
                    f"[runner] Agent '{name}' already running (use replace=true)"
                )
                return
    ...
    async def _node_heartbeat_loop(self, interval: float = 10.0):
        ...
        node_id = str(
            uuid.uuid5(uuid.NAMESPACE_DNS, f"agentflow.node.{self.node_name}")
        )
        while self._running:
            try:
    ...
    async def _publisher_loop(self):
        ...
        def _connect():
            c = paho_mqtt.Client(
                client_id=f"runner-pub-{self.node_name}-{uuid.uuid4().hex[:6]}"
            )
        ...
        while self._running:
            try:
                if client is None:
                    ...
                    logger.info(
                        f"[runner] Publisher connected to {self.broker}:{self.port}"
                    )
        ...
        while self._running:
            try:
                async with aiomqtt.Client(self.broker, self.port) as client:
                    ...
                    logger.info(
                        f"[runner] Subscribed to control topics on node '{self.node_name}'"
                    )
                        ...
                        if topic_str == f"nodes/{self.node_name}/desired_state":
                            ...
                            for agent_config in desired:
                                ...
                                if aname in self._agents:
                                    logger.info(
                                        f"[runner] '{aname}' already running, skipping."
                                    )
                                else:
                                    logger.info(
                            ...
                            asyncio.create_task(
                                self.publish(topic_str, b"", retain=True)
                            )
                ...
                if self._running:
                    logger.warning(
                        f"[runner] Subscriber disconnected: {e}. Reconnecting in 3s..."
                    )
                    await asyncio.sleep(3)
    ...
    async def run(self):
        ...
        logger.info(
            f"[runner] Starting node '{self.node_name}' → broker {self.broker}:{self.port}"
        )
    ...
    async def _migrate_agent(self, payload: dict):
        ...
        if not name or not target_node:
            logger.warning(
                "[runner] migrate: missing 'name' or 'target_node' in payload"
            )
            return
        ...
        logger.info(
            f"[runner] Migrating '{name}' from {self.node_name} → {target_node}"
        )
...
def main():
    ...
    parser.add_argument(
        "--broker",
        default=os.getenv("AGENTFLOW_BROKER", "localhost"),
...
async def _run_supervisor_tests():
    ...
    def make_agent(
        code, max_restarts=3, restart_delay=0.01, poll_interval=0.01, escalate_after=5
    ):
        runner = _StubRunner()
        config = {
            "name": "test-agent",
        ...
        agent._running = (
            True  # start() sets this; we call _supervisor_loop directly in tests
        )
    ...
    try:
        ...
        pass
    check(
        "stable: restart_count=0",
        agent._restart_count == 0,
        f"got {agent._restart_count}",
    )
