...
class MainActor(LLMAgent):
    DESCRIPTION = (
        "Main orchestrator: spawns agents, routes tasks, manages the multi-agent system"
    )
    CAPABILITIES = [
        "spawn_agent",
        "list_agents",
    ...
    async def _restore_spawned_agents(self):
        ...
        for name, config in reg.items():
            ...
            if node:
                ...
                logger.info(
                    f"[{self.name}] Re-spawning remote agent '{name}' on node '{node}'"
                )
                try:
    ...
    async def handle_message(self, msg: Message):
        if msg.type == MessageType.TASK:
            ...
            if isinstance(msg.payload, dict) and msg.payload.get(
    ...
    async def process_user_input(self, text: str) -> str:
        ...
        if stripped.startswith("/topics"):
            ...
            lines = [
                f"Known MQTT topics{' matching ' + repr(keyword) if keyword else ''}:"
            ]
            for t in topics:
                agent_strs = ", ".join(
        ...
        if stripped.startswith("/capabilities"):
            ...
            lines = [
                "Agent capabilities"
                + (" matching " + repr(keyword) if keyword else "")
                + ":"
            ]
            for a in caps:
                lines.append("")
        ...
        if text.startswith("@"):
            ...
            local_target = (
                self._registry.find_by_name(target_name) if self._registry else None
            )
            if not local_target:
                ...
                if manifest.get("spawnable") and manifest.get("catalog"):
                    ...
                    catalog_actor = (
                        self._registry.find_by_name(catalog_name)
                        if self._registry
                        else None
                    )
                    if catalog_actor and hasattr(catalog_actor, "_action_spawn"):
                        ...
                        try:
                            spawn_result = await catalog_actor._action_spawn(
                                target_name, {}
                            )
                            if spawn_result and spawn_result.get("ok"):
                                ...
                                logger.info(
                                    f"[main] '{target_name}' spawned, routing task..."
                                )
                            else:
                                err = (
                                ...
                                return (
                                    note_prefix
                                    + f"Could not spawn '{target_name}': {err}"
                                )
                        except Exception as e:
            ...
            if local_target:
                ...
                if result:
                    reply = (
                        result.get("result") or result.get("response") or str(result)
                    )
            ...
            if remote_node:
                ...
                try:
                    ...
                    return (
                        note_prefix + f"**{target_name}** (on {remote_node}): {reply}"
                    )
            ...
            known_remote = [
                a for nd in self._known_nodes.values() for a in nd.get("agents", [])
            ]
            if known_remote:
                return note_prefix + (
        ...
        if await self._is_home_automation_request(text):
            result = await self.delegate_task(
                "home-assistant-agent", text, timeout=120.0
            )
            ...
            if not result:
                return (
                    note_prefix
                    + "I could not reach the Home Assistant agent right now. Please retry."
                )
            return (
                note_prefix
                + "The Home Assistant agent did not return a result. Please retry."
            )
        ...
        asked_spawn = any(
            w in text.lower()
            for w in ("spawn", "create", "make", "build", "add", "agent")
        )
        if has_code and not has_spawn and asked_spawn:
            logger.info(
                f"[{self.name}] Code written without <spawn> — prompting to wrap it"
            )
        ...
        if spawned:
            ...
            live_names = [
                a.name for a in spawned if not isinstance(a, _SpawnPlaceholder)
            ]
            parts = []
            if live_names:
    ...
    async def process_user_input_stream(self, text: str):
        ...
        if spawned:
            ...
            if names:
                replaced = (
                    '"replace": true' in full_response
                    or '"replace":true' in full_response
                )
                parts.append(
    ...
    async def _run_planner(self, task: str) -> Optional[str]:
        ...
        await self._mqtt_publish(
            ...
            {
                "type": "log",
                "message": "Complex task detected — spawning planner...",
    ...
    async def _execute_llm_delegations(self, response: str) -> str:
        ...
        for full_match, agent_name, payload in delegations:
            ...
            if not target:
                ...
                if manifest.get("spawnable") and manifest.get("catalog"):
                    catalog_actor = (
                        self._registry.find_by_name(manifest["catalog"])
                        if self._registry
                        else None
                    )
                    if catalog_actor and hasattr(catalog_actor, "_action_spawn"):
                        logger.info(
                            f"[{self.name}] Auto-spawning '{agent_name}' via catalog..."
                        )
                        try:
                            spawn_result = await catalog_actor._action_spawn(
                                agent_name, {}
                            ...
                            if spawn_result and spawn_result.get("ok"):
                                await asyncio.sleep(0.5)
                                target = (
                                    self._registry.find_by_name(agent_name)
                                    if self._registry
                                    else None
                                )
                                logger.info(
                                    f"[{self.name}] '{agent_name}' spawned successfully"
                                )
                            else:
                                err = (
                                ...
                                logger.warning(
                                    f"[{self.name}] Spawn failed for '{agent_name}': {err}"
                                )
                        except Exception as e:
                            logger.error(
                                f"[{self.name}] Spawn error for '{agent_name}': {e}"
                            )
            ...
            logger.info(
                f"[{self.name}] Executing LLM delegation → @{agent_name} {json_str[:80]}"
            )
            try:
    ...
    def _parse_spawn_config(raw: str) -> dict:
        ...
        if bt_match:
            ...
            placeholder = re.sub(
                r'"code"\s*:\s*`.*?`', '"code": "__CODE__"', raw, flags=re.DOTALL
            )
        ...
        for match in re.findall(pattern, response, re.DOTALL):
            try:
                ...
                if agent_type == "dynamic" and not has_code:
                    logger.error(
                        f"[{self.name}] Dynamic agent has no code: {config.get('name')}"
                    )
                    continue
    ...
    async def _spawn_from_config(
        self, config: dict, save: bool = True
    ) -> Optional[Actor]:
        ...
        if existing:
            if not replace:
                logger.info(
                    f"[{self.name}] '{name}' already exists (use replace=true to update)."
                )
                return existing
        ...
        if agent_type == "manual" or name == "manual-agent":
            ...
            logger.warning(
                f"[{self.name}] Spawn config for '{name}' has neither code nor system_prompt."
            )
            return None
    ...
    async def _spawn_dynamic_agent(self, config: dict, name: str, code: str):
        ...
        if packages:
            ...
            logger.info(
                f"[{self.name}] Scheduling background install+spawn for '{name}': {packages}"
            )
    ...
    async def _install_then_spawn(
        self, config: dict, name: str, code: str, packages: list
    ):
        ...
        try:
            ...
            logger.error(
                f"[{self.name}] Background install+spawn failed for '{name}': {e}"
            )
    ...
    async def _install_packages(self, packages: list[str]):
        ...
        if not needed:
            logger.info(
                f"[{self.name}] All packages already available: {packages} — skipping install"
            )
            return
        ...
        if not installer:
            logger.warning(
                f"[{self.name}] installer agent not found — skipping install of {needed}"
            )
            return
        ...
        try:
            ...
            logger.info(
                f"[{self.name}] Install result: {result.get('message', result)}"
            )
    ...
    def list_capabilities(self, keyword: str = "") -> list[dict]:
        ...
        for name, manifest in self._agent_manifests.items():
            ...
            if kw_words:
                haystack = (
                    desc.lower() + " " + " ".join(caps).lower() + " " + name.lower()
                )
            ...
            try:
                async with aiomqtt.Client(self._mqtt_broker, self._mqtt_port) as client:
                    ...
                    async for msg in client.messages:
                        ...
                        logger.debug(
                            f"[main] Manifest from '{agent_name}': {published}"
                        )
                ...
                if self.state.value not in ("stopped", "failed"):
                    logger.warning(
                        f"[main] Manifest listener error: {e}. Reconnecting in 5s…"
                    )
    ...
    async def migrate_agent(self, agent_name: str, target_node: str) -> dict:
        ...
        if not config:
            return {
                "success": False,
                "message": f"Agent '{agent_name}' not in spawn registry.",
            }
        ...
        if current_node == target_node:
            return {
                "success": False,
                "message": f"Agent '{agent_name}' is already on '{target_node}'.",
            }

        if current_node:
            # ── Remote → Remote migration ────────────────────────────────────
            logger.info(
                f"[{self.name}] Migrating '{agent_name}' from node '{current_node}' → '{target_node}'"
            )
            await self._mqtt_publish(
            ...
            if self._registry:
                ...
                if local:
                    try:
                        ...
                        logger.warning(
                            f"[{self.name}] Could not stop local '{agent_name}': {e}"
                        )
    ...
    async def _node_heartbeat_listener(self):
        ...
        try:
            ...
            logger.warning(
                "[main] aiomqtt not available — node heartbeat tracking disabled."
            )
            return
    ...
    async def delegate_to_installer(
        self, payload: dict, timeout: float = 300.0
    ) -> dict:
        """
    ...
    async def delegate_task(
        ...
        await self.send(
            target.actor_id, MessageType.TASK, {"text": task, "reply_to": self.actor_id}
        )
        try:
            return await asyncio.wait_for(future, timeout=timeout)
        except asyncio.TimeoutError:
