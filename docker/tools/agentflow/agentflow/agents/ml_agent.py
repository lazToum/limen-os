...
class MLAgent(Actor):
    ...
    async def handle_message(self, msg: Message):
        if msg.type == MessageType.TASK:
            ...
            if not self._model_loaded:
                await self.send(
                    msg.sender_id, MessageType.RESULT, {"error": "Model not loaded"}
                )
                return
            try:
    ...
    async def _on_continuous_result(self, result: Any):
        ...
        await self._mqtt_publish(
            f"agents/{self.actor_id}/result",
            {"result": str(result), "timestamp": time.time()},
        )
...
class YOLOAgent(MLAgent):
    ...
    def __init__(
        self, model_path: str = "yolov8n.pt", confidence: float = 0.5, **kwargs
    ):
    ...
    def load_model(self):
        try:
            ...
            logger.warning(
                "[YOLOAgent] ultralytics not installed. pip install ultralytics"
            )
            return None
...
class AnomalyDetectorAgent(MLAgent):
    ...
    async def predict(self, input_data: Any) -> dict:
        ...
        if is_anomaly:
            logger.warning(
                f"[{self.name}] ANOMALY DETECTED: z={zscore:.2f} value={value}"
            )
