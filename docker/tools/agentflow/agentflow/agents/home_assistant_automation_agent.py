...
class HomeAssistantAutomationAgent(LLMAgent):
    ...
    def __init__(self, llm_provider: LLMProvider | None = None, **kwargs):
        ...
        self.ha_url = (
            os.getenv("HOME_ASSISTANT_URL") or os.getenv("HA_URL") or ""
        ).strip()
        self.ha_token = (
            os.getenv("HOME_ASSISTANT_TOKEN") or os.getenv("HA_TOKEN") or ""
        ).strip()
    ...
    async def handle_message(self, msg: Message):
        ...
        request_text, selected_entities, hardware_context = self._extract_payload(
            msg.payload
        )
        result = await self._create_and_insert(
        ...
        return {
            ...
            "automation": {
                ...
                "description": automation.get(
                    "description", "Generated by home-assistant-automation agent"
                ),
                "trigger": automation.get("trigger", []),
                "condition": automation.get("condition", []),
                "action": automation.get("action", []),
    ...
    async def _create_and_insert(
        ...
        try:
            generated = await self._create_with_llm(
                request_text, selected_entities, hardware_context
            )
            ...
            logger.error(
                f"[{self.name}] Failed to create automation: {exc}", exc_info=True
            )
            return {
                "can_create": False,
                "inserted": False,
...
def _patch_main_actor_delegate_task() -> None:
    ...
    async def _delegate_task_with_normalized_key(
