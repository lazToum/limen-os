...
class HomeAssistantHardwareAgent(LLMAgent):
    ...
    async def _get_available_devices(self) -> dict[str, Any]:
        ...
        if (
            ...
            < self._device_cache_ttl_seconds
        ):
    ...
    def _format_result(
        ...
        if connected:
            lines.append(
                "Hardware selection is based on currently discovered Home Assistant entities."
            )
        else:
        ...
        lines.append(
            "This agent only selects hardware and does not create the automation."
        )
        return {
            "can_fulfill": True,
            "task": request_text,
    ...
    async def _recommend_hardware_with_llm(
        self, request_text: str, available: dict[str, Any]
    ) -> dict[str, Any] | None:
        if self.llm is None:
            return None
        ...
        response, _ = await self.llm.complete(
            messages=[user_input], system=self.system_prompt
        )
        ...
        return self._format_result(
            request_text, available, selected, can_fulfill, fallback_text
        )
