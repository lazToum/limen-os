...
class CatalogAgent(Actor):
    ...
    async def handle_message(self, msg: Message):
        ...
        task_id = (
            payload.get("task") or payload.get("_task_id")
    ...
    async def _handle(self, payload) -> dict:
        ...
        if isinstance(payload, dict) and payload.get("action"):
            ...
            return {
                "ok": False,
                "message": f"Unknown action '{action}'. Use: spawn | list | info",
            }

        # ── Convenience dict shortcuts ─────────────────────────────────────
        if (
            isinstance(payload, dict)
            and "spawn" in payload
            and isinstance(payload["spawn"], str)
        ):
    ...
    async def _action_spawn(self, name: str, payload: dict) -> dict:
        ...
        try:
            ...
            if install:
                installer = (
                    self._registry.find_by_name("installer") if self._registry else None
                )
                if installer:
                    logger.info(
                        f"[{self.name}] Installing deps for '{name}': {install}"
                    )
                    import uuid as _uuid

                    task_id = f"cat_install_{_uuid.uuid4().hex[:8]}"
                    future = asyncio.get_event_loop().create_future()
                    installer._result_futures = getattr(
                        installer, "_result_futures", {}
                    )
                    ...
                    logger.warning(
                        f"[{self.name}] installer agent not found — skipping dep install for '{name}'"
                    )
            ...
            persistence_dir = (
                str(getattr(main, "_persistence_dir", "./state/main").parent)
                if main
                else "./state"
            )
