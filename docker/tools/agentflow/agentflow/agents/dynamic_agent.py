...
class DynamicAgent(Actor):
    ...
    async def on_start(self):
        ...
        if error_msg:
            for attempt in range(1, self._MAX_COMPILE_RETRIES + 1):
                ...
                if new_err is None:
                    ...
                    error_msg = None
                    logger.info(
                        f"[{self.name}] Code fixed by LLM after {attempt} attempt(s)."
                    )
        ...
        if error_msg:
            ...
            logger.error(
                f"[{self.name}] Code compilation failed permanently: {error_msg}"
            )
    ...
    def _sanitize_code(code: str) -> str:
        ...
        def collect_block(
            lines, start, base_indent, conts=("except", "else", "finally", "elif")
        ):
            j, block = start, []
        ...
        while i < len(lines):
            ...
            if fn_m:
                ...
                result += [
                    ...
                    p
                    + "    return await agent.llm.complete(messages=msgs, system=sys_p)",
                ]
                last_sanitized = False
                i = j
                continue
    ...
    def _compile_code(self, code: Optional[str] = None) -> Optional[str]:
        ...
        try:
            ...
            fns = [
                f
                for f in ["setup", "process", "handle_task", "cleanup"]
                if f in self._ns
            ]
    ...
    async def _fix_syntax_with_llm(
        self, bad_code: str, error_msg: str
    ) -> Optional[str]:
        """
        ...
        if msg.type == MessageType.TASK:
            ...
            if self._fn_handle_task:
                try:
                    ...
                    await self._publish_error(
                        phase="handle_task", error=e, traceback_str=tb
                    )
                    if msg.sender_id:
                        await self.send(
                            msg.sender_id,
...
def _ensure_result_handler(actor):
    ...
    original = (
        actor.handle_message.__func__
        if hasattr(actor.handle_message, "__func__")
        else None
    )
...
class _AgentAPI:
    ...
    async def publish_detection(self, data: Any):
        ...
        await self._actor._mqtt_publish(
            f"agents/{self._actor.actor_id}/detections", data
        )
    ...
    async def send_to(
        self, agent_name: str, payload: Any, timeout: float = 60.0
    ) -> Optional[Any]:
        ...
        try:
            ...
            logger.warning(
                f"[{self.name}] send_to '{agent_name}' timed out after {timeout}s"
            )
    ...
    async def send_to_many(
        self, tasks: list[tuple[str, Any]], timeout: float = 60.0
    ) -> list:
    ...
    def nodes(self) -> list[dict]:
        ...
        """
        main = (
            self._actor._registry.find_by_name("main")
            if self._actor._registry
            else None
        )
        ...
        async def _fetch():
            try:
                async with aiomqtt.Client(
                    actor._mqtt_broker, actor._mqtt_port
                ) as client:
