...
class ManualAgent(Actor):
    ...
    async def on_start(self):
        await self._mqtt_publish(
            ...
            {
                "type": "log",
                "message": "Manual agent ready. Send {action: load_manual, device: ...} to begin.",
                "timestamp": time.time(),
            },
        )
    ...
    async def _load_manual(self, device: str) -> dict:
        ...
        pdf_url = await loop.run_in_executor(
            None, lambda: self._search_for_manual(device)
        )
        ...
        text, pages = await loop.run_in_executor(
            None, lambda: self._extract_text(pdf_bytes)
        )
        if not text:
            return {
                "error": "PDF has no extractable text (may be a scanned image PDF)."
            }
    ...
    def _search_for_manual(self, device: str) -> Optional[str]:
        try:
            ...
            logger.error(
                f"[{self.name}] httpx is not installed — cannot search for manuals"
            )
            return None
                    ...
                    for url in direct_urls:
                        try:
                            ...
                            if r.status_code == 200 and (
                                "pdf" in ct or url.endswith(".pdf")
                            ):
                                logger.info(f"[{self.name}] Direct URL works: {url}")
                                return url
                        except Exception as e:
                            logger.debug(
                                f"[{self.name}] Direct URL failed ({url}): {e}"
                            )
                            continue
            except Exception as e:
        ...
        logger.warning(
            f"[{self.name}] All search passes exhausted — no manual found for: {device}"
        )
        return None
    ...
    def _search_ddgs(self, device: str) -> Optional[str]:
        ...
        try:
            try:
                ...
                logger.info(
                    f"[{self.name}] Pass 2: using duckduckgo_search (deprecated)"
                )
            ...
            with DDGS() as ddgs:
                for query in queries:
                    try:
                        ...
                        logger.info(
                            f"[{self.name}] Pass 2 query: {query!r} → {len(results)} results"
                        )
                        ...
                        logger.warning(
                            f"[{self.name}] DDGS query failed ({query}): {e}"
                        )
                        continue
        except ImportError:
            logger.warning(
                f"[{self.name}] Neither ddgs nor duckduckgo_search installed — skipping"
            )
    ...
    def _search_bing_scrape(self, device: str, headers: dict) -> Optional[str]:
        ...
        try:
            with httpx.Client(
                ...
                for query in queries:
                    try:
                        url = "https://www.bing.com/search?q=" + urllib.parse.quote(
                            query
                        )
                        r = client.get(url)
                        ...
                        logger.info(
                            f"[{self.name}] Pass 3 query: {query!r} → {len(urls)} real URLs"
                        )
                        ...
                        logger.warning(
                            f"[{self.name}] Bing query failed ({query}): {e}"
                        )
                        continue
        except Exception as e:
            ...
            with httpx.Client(
                ...
                for query in queries:
                    try:
                        ...
                        logger.info(
                            f"[{self.name}] Pass 4 query: {query!r} → {len(urls)} real URLs"
                        )
                        for i, u in enumerate(urls[:10]):
                            logger.info(f"[{self.name}]   [{i}] {u}")
                        ...
                        logger.warning(
                            f"[{self.name}] Google query failed ({query}): {e}"
                        )
                        continue
        except Exception as e:
    ...
    def _extract_bing_urls(html: str) -> list[str]:
        ...
        for m in re.finditer(
            r'href="https?://www\.bing\.com/ck/a\?[^"]*?u=a1([A-Za-z0-9_-]+)[^"]*"',
            html,
        ):
            try:
                encoded = m.group(1)
    ...
    def _extract_google_urls(html: str) -> list[str]:
        ...
        for m in re.finditer(r'/url\?q=(https?://[^&"]+)', html):
            try:
                ...
                if (
                    not any(d in decoded for d in _SEARCH_ENGINE_DOMAINS)
        ...
        try:
            async with httpx.AsyncClient(
                ...
                if resp.status_code != 200:
                    logger.warning(
                        f"[{self.name}] Download returned status {resp.status_code} for: {url}"
                    )
                    return None
                ...
                links = re.findall(
                    r'https?://[^\s"\'<>]+\.pdf', resp.text, re.IGNORECASE
                )
                if links:
                    logger.info(
                        f"[{self.name}] Following embedded PDF link: {links[0]}"
                    )
                    r2 = await client.get(links[0])
                    if r2.status_code == 200 and r2.content[:4] == b"%PDF":
                        return r2.content
                logger.warning(
                    f"[{self.name}] URL did not return a PDF: {url} (content-type: {ct})"
                )
        except Exception as e:
    ...
    def _extract_text(self, pdf_bytes: bytes) -> tuple[str, int]:
        ...
        try:
            ...
            logger.error(
                f"[{self.name}] Neither pdfplumber nor pymupdf (fitz) installed — cannot extract text"
            )
        except Exception as e:
