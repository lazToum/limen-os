...
class InstallerAgent(Actor):
    ...
    async def _install_packages(self, packages: list[str]) -> dict:
        ...
        for pkg in packages:
            ...
            if not success and pip_name == "pdfplumber":
                logger.info(
                    f"[{self.name}] pdfplumber failed, trying pymupdf as fallback..."
                )
        ...
        try:
            async with asyncssh.connect(**self._ssh_kwargs(payload)) as conn:
                ok, output = await self._ssh_run(
                    conn, f"pip install {pkg_str} --break-system-packages -q 2>&1"
                )
                if ok:
    ...
    async def _node_deploy(self, payload: dict) -> dict:
        ...
        if not runner_path:
            return {
                "error": "remote_runner.py not found. Make sure it is in the agentflow root."
            }
        ...
        try:
            async with asyncssh.connect(**self._ssh_kwargs(payload)) as conn:
                ...
                async with conn.start_sftp_client() as sftp:
                    await sftp.put(
                        str(runner_path), f"/home/{user}/agentflow/remote_runner.py"
                    )
