...
class MonitorActor(Actor):
    ...
    async def on_start(self):
        ...
        logger.info(
            f"[{self.name}] Monitor started. check_interval={self.check_interval}s"
        )
    ...
    async def _check_all_actors(self):
        ...
        for actor in self._registry.all_actors():
            ...
            if gap > self.heartbeat_timeout and actor.state == ActorState.RUNNING:
                ...
                if (
                    self._alert_state.get(actor.actor_id)
                    and gap <= self.heartbeat_timeout
                ):
    ...
    async def _handle_error_event(self, event: dict):
        ...
        if fatal:
            ...
            if actor and self._restart_attempts.get(actor_id, 0) < 3:
                self._restart_attempts[actor_id] = (
                    self._restart_attempts.get(actor_id, 0) + 1
                )
                restarted = await self._attempt_restart(
                    actor,
                    reason=f"{phase} error (attempt {self._restart_attempts[actor_id]})",
                )
                if restarted:
                    msg = (
    ...
    async def _check_error_registry(self):
        ...
        for actor_id, event in list(self._error_registry.items()):
            ...
            if event.get("degraded") and (now - last) > _NOTIFY_COOLDOWN:
                ...
                if (
                    actor
                    and hasattr(actor, "_consecutive_errors")
                    and actor._consecutive_errors == 0
                ):
    ...
    async def _notify_main(
        ...
        try:
            ...
            logger.info(
                f"[{self.name}] Notified main about '{agent_name}': {message[:80]}"
            )
        except Exception as e:
