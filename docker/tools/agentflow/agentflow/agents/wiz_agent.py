...
class WizAgent(Actor):
    ...
    async def _economy_listener(self) -> None:
        try:
            ...
            logger.warning(
                "[%s] aiomqtt not available — economy listener disabled", self.name
            )
            return
    ...
    def _cmd_history(self, n: int) -> str:
        ...
        for e in reversed(self._history[-n:]):
            ...
            rows.append(
                f"  `{t}` {sign}{e['delta']} Ƿ — {e['reason']} (bal: {e['balance']})"
            )
        return (
