"""
SmartCitiesAgent — LIMEN integration for smart city sensors.

Subscribes to LIMEN's Fuseki triple store and MQTT bus to:
...
MQTT topics consumed:
    limen/smartcities/safety/alerts        — weapon alerts from MQTT bridge
    limen/telemetry/+                      — telemetry from any sensor
    limen/smartcities/+/+/rdf              — raw RDF from bridge

MQTT topics published:
    limen/smartcities/status               — periodic city state summary
    limen/smartcities/road-density         — road user count by zone
    limen/alerts/weapon                    — weapon alert mirror
    agents/<id>/logs                         — agent log (via standard framework)

Environment:
    FUSEKI_URL       Fuseki base URL (default: http://fuseki:3030)
    FUSEKI_DATASET   Fuseki dataset (default: limen)
...
_FUSEKI_DATASET = os.getenv("FUSEKI_DATASET", "limen")
...
# Road user counts by sensor (latest via urn:limen:current)
_ROAD_DENSITY_QUERY = """\
{prefixes}
...
class SmartCitiesAgent(Actor):
    """
    AgentFlow actor that bridges LIMEN's Fuseki triple store with the
    ...
    async def on_start(self):
        ...
        logger.info(
            "[%s] started — poll interval: %ds, weapon threshold: %.2f",
            self.name,
    ...
    async def _check_weapons(self, since: str):
        ...
        for row in rows:
            ...
            await self._mqtt_publish("limen/alerts/weapon", alert)
            await self._mqtt_publish("limen/smartcities/safety/alerts", alert)
            await self._mqtt_publish(
    ...
    async def _publish_road_density(self):
        ...
        await self._mqtt_publish(
            "limen/smartcities/road-density",
            {
                "by_sensor": density,
                "total": total,
                "timestamp": _iso_now(),
    ...
    async def _dispatch(self, text: str, payload: dict) -> str:
        ...
        if lower.startswith("query ") or lower.startswith("sparql "):
            sparql = (
                text[6:].strip() if lower.startswith("query ") else text[7:].strip()
            )
        ...
        if "weapon" in lower or "alert" in lower or "armed" in lower:
            ...
            lines = [
                f"**Weapon detections** (last 1h, confidence ≥ {self._weapon_thresh}):\n"
            ]
            for r in rows:
                ...
                lines.append(
                    f"- {sev} sensor=`{sensor}` confidence=`{conf:.2f}` @ {ts[:19]} (WID: {wid})"
                )
            return "\n".join(lines)
        ...
        if "parking" in lower or "occupancy" in lower:
            ...
            for r in rows:
                ...
                total = _val(r, "total")
                pct = (
                    round(int(occupied) / int(total) * 100) if int(total or "0") else 0
                )
...
_HELP = """\
**SmartCities Agent** 🏙️
_LIMEN smart city sensor monitor_
...
**Integration points:**
- Polls `urn:limen:events` in Fuseki every 30s for new weapon detections
- Publishes alerts to `limen/alerts/weapon` and `limen/smartcities/safety/alerts`
- Road density → `limen/smartcities/road-density`
- Parking summary → `limen/smartcities/status`
