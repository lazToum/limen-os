...
class WifAgent(Actor):
    ...
    def _cmd_add(self, args: list[str]) -> str:
        ...
        header = f"Logged **${amount:.2f}** → `{category}`" + (
    ...
    def _cmd_budget(self, args: list[str]) -> str:
        ...
        return (
            f"Budget set: **${amount:.2f}** | Spent so far: ${spent:.2f} ({pct:.0f}%)"
        )
    ...
    def _cmd_loan(self, args: list[str]) -> str:
        if len(args) < 3:
            return (
                "Usage: `loan <principal> <rate%> <months>`  e.g. `loan 20000 5.5 60`"
            )
        try:
            p, annual, n = (
                float(args[0].lstrip("$")),
    ...
    def _cmd_tip(self, args: list[str]) -> str:
        ...
        total = bill + tip
        splits = "\n".join(
            f"  {n} people: ${total / n:.2f}/person" for n in (1, 2, 3, 4, 5)
        )
