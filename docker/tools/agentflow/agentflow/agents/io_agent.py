...
class IOAgent(Actor):
    ...
    async def _io_chat_listener(self):
        ...
        try:
            ...
            logger.error(
                f"[{self.name}] aiomqtt not installed — io/chat listener disabled"
            )
            return
        ...
        while self.state not in (ActorState.STOPPED, ActorState.FAILED):
            try:
                ...
                if self.state not in (ActorState.STOPPED, ActorState.FAILED):
                    logger.warning(
                        f"[{self.name}] io/chat disconnected: {exc}. Retry in 5s"
                    )
    ...
    async def _route_chat(self, data: dict):
        ...
        logger.info(
            f"[{self.name}] routing from '{from_id}' → '{target.name}': {text[:60]!r}"
        )
        ...
        if target_name in ("main-actor", "main") and hasattr(
            target, "process_user_input_stream"
        ):
            buf = []
    ...
    async def _slash_agents(self):
        ...
        for actor in self._registry.all_actors():
            ...
            lines.append(
                f"  [{state:8s}] @{actor.name:<22s} {actor.actor_id[:8]}{protected}{node}"
            )
        await self._reply(
    ...
    async def _slash_nodes(self):
        ...
        lines = [
            f"  {'local':20s} online   {', '.join('@' + n for n in local_agents) or '(none)'}"
        ]
    ...
    async def _slash_deploy(
        self, node_name: str, host: str, user: str, pw: str, broker: str
    ):
        # ── Step 1: discover host if not provided ──────────────────────────
        if not host:
            await self._reply(
                f"[discover] Searching for '{node_name}' on the network..."
            )
