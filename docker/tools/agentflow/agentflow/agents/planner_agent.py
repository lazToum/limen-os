...
class PlannerAgent(Actor):
    ...
    async def handle_message(self, msg: Message):
        if msg.type == MessageType.TASK:
            payload = (
                ...
                else {"text": str(msg.payload)}
            )
            task_text = payload.get("text") or payload.get("task") or str(msg.payload)
            self._reply_to_id = (
                payload.get("_reply_to")
                or msg.reply_to
                or msg.sender_id
                or self._reply_to_id
            )
    ...
    def _load_cached_plan(self, cache_key: str, workers: list[dict]) -> Optional[list]:
        ...
        for step in plan:
            ...
            if agent not in alive and not step.get("spawn_config"):
                logger.info(
                    f"[{self.name}] Cache invalid — agent '{agent}' no longer running"
                )
                return None
    ...
    def _save_plan_cache(self, cache_key: str, task: str, plan: list):
        ...
        now = time.time()
        raw = {
            k: v for k, v in raw.items() if now - v.get("timestamp", 0) < _CACHE_TTL_S
        }
        raw[cache_key] = {
            "task": task[:200],
            "plan": plan,
  ...
  {{"step": 3, "agent": "main", "task": "Summarize the weather and news results", "parallel": false, "depends_on": [1, 2], "spawn_config": null}}
    ...
    async def _ensure_agents(self, plan: list[dict]) -> list[dict]:
        ...
        for step in plan:
            ...
            if existing:
                await self._log(
                    f"Agent '{agent_name}' already running — skipping spawn"
                )
    ...
    async def _spawn_agent(self, config: dict) -> Optional[Actor]:
        ...
        if agent_type == "dynamic":
            ...
            if not code:
                logger.warning(
                    f"[{self.name}] Dynamic spawn config has no code for '{name}'"
                )
                return None
    ...
    async def _delegate(
        ...
        if not target:
            logger.warning(
                f"[{self.name}] Agent '{agent_name}' not found for delegation"
            )
