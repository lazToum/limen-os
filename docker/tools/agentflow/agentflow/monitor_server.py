...
async def _slash_deploy(
    node: str, host: str, user: str, pw: str, broker: str, reply_fn
):
    if not host:
        ...
        if not discovered:
            try:
                ...
                subnet = "192.168.1"
            await reply_fn(
                f"[discover] mDNS not found. Scanning {subnet}.1-254 for SSH..."
            )
...
async def handle_slash(text: str, reply_fn) -> bool:
    ...
    if cmd == "/agents":
        ...
        for actor in registry.all_actors():
            ...
            lines.append(
                f"  [{st:8s}] @{actor.name:<22s} {actor.actor_id[:8]}{protected}{node}"
            )
        await reply_fn(
    ...
    if cmd == "/nodes":
        main = _find_main()
        remote_nodes = (
            main.list_nodes() if (main and hasattr(main, "list_nodes")) else []
        )
        local = [a.name for a in registry.all_actors()] if registry else []
        lines = [
            f"  {'local':20s} online   {', '.join('@' + n for n in local) or '(none)'}"
        ]
...
async def handle_chat_mqtt(data: dict):
    ...
    await _route_chat(
        content, mqtt_reply
    )  # MQTT path: no streaming, reply_fn used for all output
                        ...
                        if content and registry is not None:
                            ...
                            async def _safe_route(c=content):
                                try:
                                    ...
                                    logger.error(
                                        f"[ws] chat error: {exc}", exc_info=True
                                    )
                                    try:
...
async def handle_command(cmd: dict):
    ...
    try:
        ...
        if command in ("stop", "pause", "resume"):
            ...
            await broadcast(
                {"type": "delete_agent", "agent_id": agent_id, "state": _snapshot()}
            )
    except Exception as e:
...
def parse_topic(topic: str, payload_str: str):
    ...
    if parts[0] == "agents" and len(parts) >= 3:
        ...
        if metric == "status":
            ...
            if isinstance(data, dict):
                state["agents"][agent_id]["messages_processed"] = data.get(
                    "messages_processed", 0
                )
                if "cost_usd" in data:
                    state["agents"][agent_id]["cost_usd"] = data.get("cost_usd", 0.0)
                    state["agents"][agent_id]["input_tokens"] = data.get(
                        "input_tokens", 0
                    )
                    state["agents"][agent_id]["output_tokens"] = data.get(
                        "output_tokens", 0
                    )
            ...
            add_log(
                {"type": "completed", "agent_id": agent_id, "timestamp": time.time()}
            )
        elif metric == "alert":
            if isinstance(data, dict):
            ...
            name = state["agents"].get(agent_id, {}).get("name", agent_id[:8])
            severity = (
                data.get("severity", "warning") if isinstance(data, dict) else "warning"
            )
            add_log(
                {
                    "type": "alert",
    ...
    if parts[0] == "nodes" and len(parts) >= 3 and parts[2] == "heartbeat":
        ...
        if isinstance(data, dict):
            ...
            logger.info(
                f"[MQTT] Node heartbeat: {node_name} | agents: {data.get('agents', [])}"
            )
                ...
                async for message in client.messages:
                    ...
                    if topic == "io/chat":
                        if registry is not None:
                            try:
                                asyncio.create_task(
                                    handle_chat_mqtt(json.loads(payload))
                                )
                            except Exception as exc:
...
async def _check_mqtt() -> bool:
    ...
    try:
        ...
        logger.error(
            f"[startup] MQTT broker {MQTT_BROKER}:{MQTT_PORT} unreachable — {exc}"
        )
        return False
            ...
            if parent.is_dir():
                for sub in sorted(parent.iterdir()):
                    if sub.is_dir() and (sub / "index.html").exists():
                        raise web.HTTPFound(
                            request.path.rstrip("/") + f"/{sub.name}/index.html"
                        )
...
async def _bridge_mqtt_tcp(client_ws, broker: str, port: int) -> None:
    ...
    try:
        ...
        logger.warning(
            "MQTT TCP bridge: cannot connect to %s:%s — %s", broker, port, exc
        )
        return
        ...
        async with aiohttp.ClientSession() as session:
            async with session.ws_connect(
                ...
                await asyncio.gather(
                    forward(client_ws, upstream_ws), forward(upstream_ws, client_ws)
                )
        return client_ws
    except Exception as exc:
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--mqtt-ws-port", type=int, default=int(os.getenv("MQTT_WS_PORT", "9001"))
    )
