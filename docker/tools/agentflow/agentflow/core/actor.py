...
class Actor(ABC):
    ...
    def __init__(
        ...
        if actor_id:
            ...
            self.actor_id = str(
                uuid.uuid5(uuid.NAMESPACE_DNS, f"agentflow.actor.{name}")
            )
        else:
        ...
        self._registry: Optional["ActorRegistry"] = None  # noqa: F821
    ...
    async def _heartbeat_loop(self, interval: float = 10.0):
        ...
        await self._mqtt_publish(
            f"agents/{self.actor_id}/heartbeat", self._build_heartbeat()
        )
        await self._mqtt_publish(
            f"agents/{self.actor_id}/metrics", self._build_metrics()
        )
        while self.state not in (ActorState.STOPPED, ActorState.FAILED):
            try:
                ...
                async with aiomqtt.Client(self._mqtt_broker, self._mqtt_port) as client:
                    ...
                    async for message in client.messages:
                        try:
                            ...
                            if self.protected and command in (
                                "stop",
                                "pause",
                                "delete",
                            ):
                            ...
                            if command == "stop":
                                ...
                                if self._registry:
                                    ...
                                    if main and hasattr(
                                        main, "_remove_from_spawn_registry"
                                    ):
    ...
    async def send(
        self, target_id: str, msg_type: MessageType, payload: Any = None
    ) -> bool:
        ...
        if self._mqtt_client:
            try:
                await self._mqtt_client.publish(
                    topic, json.dumps(payload), retain=retain, qos=qos
                )
            except Exception as e:
    ...
    def __repr__(self):
        return (
            f"<Actor name={self.name} id={self.actor_id[:8]} state={self.state.value}>"
        )
