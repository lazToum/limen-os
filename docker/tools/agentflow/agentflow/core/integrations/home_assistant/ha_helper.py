...
async def fetch_devices_entities_with_location(
    ...
    async with HAWebSocketClient(ws_url, token) as ha:
        ...
        for d in devices:
            ...
            device_area_name = (
                area_name_by_id.get(device_area_id) if device_area_id else None
            )
            ...
            for e in entities_by_device.get(device_id, []):
                ...
                entity_area_name = (
                    area_name_by_id.get(entity_area_id) if entity_area_id else None
                )
...
async def _fetch_automation_config(
    rest_base: str, automation_id: str, token: str
) -> dict[str, Any] | None:
    headers = {
        "Authorization": f"Bearer {token}",
        "Content-Type": "application/json",
...
async def create_automation_via_rest(
    ...
    slug_base = (
        re.sub(r"[^a-z0-9]+", "_", alias.lower()).strip("_") or "generated_automation"
    )
